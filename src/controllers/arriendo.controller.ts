...
import { Arriendo } from "../interface/post.interface";
import { promises } from "dns";

export async function getArriendo(req: Request, res: Response): Promise<Response> {

    const conn = await connect();
    const arriendo = await conn.query('SELECT * FROM  arriendo');
    return res.json(arriendo[0])

};

export async function createArriendo(req: Request, res: Response) {

    const newPost: Arriendo = req.body;
    const conn = await connect();
    await conn.query('INSERT INTO arriendo  SET?', [newPost]);
...
export async function updateArriendo(req: Request, res: Response) {
    ...
    const id = req.params.arriendoId;
    const updateArriendo: Arriendo = req.body;
    const conn = await connect();
